...
var RuntimePackage = require('./runtime.js')
var Runtime = RuntimePackage.Runtime
var Facade = RuntimePackage.Facade
...
class Bus{
    constructor(options){
        this.options = util.defaults( options || {}, {defaultModule:'_system'})
    ...
    normalizeEvent( rawEvent ){
        ...
        if( !fireEvent.muteBy ){
            ...
            if( this._disable.get(fireEvent.name) ){
                ...
                for( let item of this._disable.get(fireEvent.name)){
    ...
    storeRuntimeMute( muteObjects ){
        ...
        muteObjects.forEach(function(mute){
            if( !that._runtime.mute.get(mute.target) ){
                that._runtime.mute.set(mute.target,new MuteRecord())
            }

            that._runtime.mute.get(mute.target).add(mute)
        })
    ...
    findListenersForEvent( eventName ){
        ...
        for( let item of this._rexEventListenerMap ){
            ...
            if( (new RegExp(rex)).test(eventName) ){
                ...
                listeners.insert( rexEventListener.indexName, _.clone(rexEventListener ), order  )
            }
        }
    ...
    callListener( listener, snapshot, args ){
        ...
        if( isGenerator(listener.fn) ){
            return co(function *(){
                yield listener.fn.apply(snapshot, args)
            })
        }else{
        ...
        var firePromise = co(function *(){
            ...
            while(  listenerCursor = listenerCursor.next ){
                ...
                if( listener.waitFor  ) {
                    ...
                    let promiseToWait = util.from(listener.waitFor).reduce(function(list,waitForName){
                    ...
                    result = this.parseResult( Promise.all(promiseToWait).then(function(){
                        return that.callListener( listenerSnapshot, snapshot, data )
                    }))
                }else{
                    //如果监听器没有waitFor
            ...
            var that = this
            _.forEach(results,function(result,listenerIndexName){
                that._runtime.stack[event._stackIndex].listeners[listenerIndexName].result = results[listenerIndexName]
            })
    ...
    bindThisToResolver(promise){
        ...
        promise.then = function(){
            ...
            var that = this
            args = args.map( function(arg){ return arg.bind(that)})
    ...
    snapshot(event, listener){
        ...
        var liftedMuteClone = new Map(util.from(this._runtime.mute).map(function(item){
            item[1].clone().lift()
            return item
        }))
    ...
    makeEventStack(event, listenersOrderedList ){
        ...
        var clonedListenerArray =  _.cloneDeep(listenersOrderedList.toArray(), function(item){
            if( item instanceof Set ){
                return util.from(item)
            }else if( item instanceof Map){
                return _.zipObject( util.from(item.keys()),util.from(item.values()))
        ...
        eventStack.listeners = _.zipObject( clonedListenerArray.map(function(listener){
