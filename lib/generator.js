...
class Bus {
  ...
  fire(rawEvent) {
    ...
    if (this.isEventMuted(event.name)) {
      //TODO tell user who muted current event
      return Promise.resolved()
    }

    const data = util.slice(arguments, 1)
  ...
  fireListeners(event, listeners, data) {
    ...
    const firePromise = co(function *() {
      ...
      while (listenerCursor.next !== undefined) {
        ...
        //if we have to waitFor other listeners
        if (listener.waitFor) {
        ...
        if (result.signal.blockFor) {
          [].concat(result.signal.blockFor).forEach(function (blockForName) {
            ...
            if (listenerToBlock) {
              ...
              debug.warn('listener not called in this event', blockForName)
            }
          })
        }
      ...
      util.forEach(results, (result, listenerIndexName)=> {
        this._runtime.storeResult(this._listenerRuntimeKey, results[ listenerIndexName ])
      })
      ...
      if (asyncErrors.length !== 0) {
        ...
        return new BusResult(this._runtime.getData(this._eventRuntimeKey), asyncErrors)
      }
  ...
  isEventMuted(eventName) {
    return false
    //const eventRuntime = this._runtime.getRuntime( this._eventRuntimeKey )
    //if( eventName.childEventsToMute.indexOf( eventName ) !== )
  }

  muteEvents(muteEventNames, type, source, inEventName) {
    muteEventNames = [].concat(muteEventNames)
  ...
  normalizeEvent(rawEvent) {
    ...
    const fireEvent = util.isString(rawEvent) ? { name: rawEvent } : rawEvent
