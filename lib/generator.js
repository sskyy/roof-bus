...
class Bus{
    ...
    fireListeners( event, listeners, data ){
        ...
        //注意 snapshot 数据是共享的
        var runtimeData = this._runtime.data.child()
        var asyncErrors = []
        ...
        var firePromise = co(function *(){
            ...
            while(  listenerCursor = listenerCursor.next ){
                ...
                let snapshot = this.snapshot(event, listener,runtimeData)
                //保存当前结果
                let result
                ...
                if( listener.waitFor  ) {
                    ...
                    if( listener.async ){
                        //async 不能是 generator
                        result = this.parseResult( this.callListener(listener, snapshot, data) )
                    }else{
                        result = this.parseResult( yield this.callListener(listener, snapshot, data) )
                    }


                    //如果是开发者自己返回的error，那么和error一样直接中断循环
            ...
            //支持async 参数 所有监听器都已经开始执行，等所有异步的的都返回后再一起resolve
            return Promise.all( util.map(results, function( result ){
                if( result.data instanceof  Promise){
                    //包装错误,保障即使有错误，也是全部都执行了再返回
            ...
            if( asyncErrors.length !== 0 ){
                ...
                return new BusResult(runtimeData, asyncErrors)
            }
