...
class OrderedList {
  constructor(list) {
    ...
    list.forEach(function(itemArgs){
      that.insert.apply(that, itemArgs)
    })
  }
  //Thanks for IE8's funny Object.defineProperty!!!!!!
  //  get length(){
    ...
    Object.keys(orderNamePair).forEach(function(orderName){
      ...
      if(normalizedOrder[orderName] ){
        ...
        for( let orderTargetKey of orderValues){
          //有我等的对象，也在等我
          if(  that._waitList[key] && that._waitList[key][orderTargetKey] ){
            if( that._waitList[key][orderTargetKey][orderName].has(key) ){
              //如果我们互相before或者互相after
              throw new Error( `${key} has conflict order  ${orderName} with ${orderTargetKey}`)
            }else if(that._waitList[key][orderTargetKey][reverseOrderName].has(key)){
  ...
  lineUp(obj){
    ...
    var that = this
    waitForKeys = new Set(waitForKeys.filter(function(key){ return !that._list.has(key)}))

    obj._waiting = waitForKeys

    var that = this
    util.from(obj._waiting).forEach(function(waitForKey){
      if( !that._waitList.has(waitForKey) ){
        that._waitList.set(waitForKey , new Map)
      }
      //console.log("setting wailList", obj.key,obj)
      that._waitList.get(waitForKey).set(obj.key,obj)
    })
  ...
  isDependenciesLoaded(order){
    ...
    return dependencies.every(function(dependency){
      return that._list.get(dependency)
    })
  }
  isReady(){
